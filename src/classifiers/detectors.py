...
def fixation_detection(x: numpy.ndarray, y: numpy.ndarray, time: numpy.ndarray, maxdist=25, mindur=50) -> List[Fixation]:
    ...
    for i in range(1, len(x)):
        ...
        if dist <= maxdist and not fixstart:
            ...
            if time[i - 1] - Sfix[-1][0] >= mindur:
                Efix.append(Fixation(Sfix[-1][0], time[i - 1], time[i - 1] - Sfix[-1][0], x[si], y[si]))
...
def saccade_detection(x: numpy.ndarray, y: numpy.ndarray, time: numpy.ndarray, minlen=5, maxvel=250, maxacc=340) -> List[Saccade]:
    ...
    while not stop:
        ...
        if len(sacstarts) > 0:
            ...
            if len(sacends) > 0:
                ...
                if dur >= minlen:
                    # add to saccade ends
                    Esac.append(Saccade(t1, t2, dur, x[t1i], y[t1i], x[t2i], y[t2i]))
                else:
