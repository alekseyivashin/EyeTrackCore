...
from classifiers.fixation import Fixation
from classifiers.saccade import Saccade
...
def draw_fixations(fixations: List[Fixation], dispsize: Tuple[int, int], imagefile=None, durationsize=True,
                   durationcolour=True, alpha=0.5,
                   savefilename=None) -> Any:
...
def draw_heatmap(fixations: List[Fixation], dispsize: Tuple[int, int], imagefile=None, durationweight=True, alpha=0.5,
                 savefilename=None):
...
def draw_raw(x: numpy.ndarray, y: numpy.ndarray, dispsize: Tuple[int, int], imagefile=None, savefilename=None):
...
def draw_scanpath(fixations: List[Fixation], saccades: List[Saccade], dispsize: Tuple[int, int], imagefile=None, alpha=0.5, savefilename=None):
    ...
    fix = parse_fixations(fixations)
    # draw fixations
    ax.scatter(fix['x'], fix['y'], s=fix['dur'], c=COLS['chameleon'][2], marker='o', cmap='jet', alpha=alpha,
               edgecolors='none')
    ...
    if saccades:
        ...
        for s in saccades:
            ...
            ax.arrow(s.start_x, s.start_y, s.end_x - s.start_x, s.end_y - s.start_y, alpha=alpha, fc=COLS['aluminium'][0], ec=COLS['aluminium'][5], fill=True,
...
def parse_fixations(fixations: List[Fixation]):
    ...
    dispsize        - screen size for normalize fixations
    ...
    for fixation_index in range(len(fixations)):
        fix['x'][fixation_index] = fixations[fixation_index].end_x
        fix['y'][fixation_index] = fixations[fixation_index].end_y
        fix['dur'][fixation_index] = fixations[fixation_index].duration
